...
public class NthPostOrderTraversal {
	...
	public static void main(String[] args) {
		...
		int index = 2;
		int counter[] = { 0 };
		nthPostOrderTraversal(head, index, counter);
	}

	private static void nthPostOrderTraversal(Node head, int index, int[] counter) {

		if (head != null) {
			nthPostOrderTraversal(head.lChild, index, counter);
			nthPostOrderTraversal(head.rChild, index, counter);
			counter[0]++;
			if (counter[0] == index) {
