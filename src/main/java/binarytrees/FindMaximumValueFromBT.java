...
public class FindMaximumValueFromBT {
	...
	public static void main(String[] args) {
		...
		System.out.println(maxValueinBT(head));
	}

	private static int maxValueinBT(Node head) {

		if (head == null) {
			return Integer.MIN_VALUE;
		}
		int max = head.data;
		int leftMax = maxValueinBT(head.lChild);
		int rightMax = maxValueinBT(head.rChild);
		if(leftMax > max) {
			max = leftMax;
		}
		if(rightMax > max) {
